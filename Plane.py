...
class Prop(Engine):
    ...
    def buildProp(self,win):
        ...
        blade = Rectangle(Point(270,351-self.size),Point(280,270 - self.size * 5))
        blade.draw(win)
        blade2 = Rectangle(Point(270,442+self.size* 5),Point(280,361 + self.size))
        blade2.draw(win)
        blade.setFill("grey")
        blade2.setFill("grey")

        blade2ndEng = Rectangle(Point(720,351-self.size),Point(730,270 - self.size * 5))
        blade2ndEng.draw(win)
        blade2ndEng2 = Rectangle(Point(720,442+self.size * 5),Point(730,361 + self.size))
        blade2ndEng2.draw(win)
        blade2ndEng.setFill("grey")
        blade2ndEng2.setFill("grey")
class Plane(object):
    def __init__(self,win):
        fuselage = Fuselage("blue",win)
        wing = Wings(win)
